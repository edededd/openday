...
def process_frame(input_frame):
    ...
    for bbox, person_id, mask in person_data:
        if person_id in active_users and active_users[person_id].is_active:
            ...
            if mask is not None:
                # Crear una máscara de color para la persona
                color_mask = np.zeros(input_frame.shape, dtype=np.uint8)
                color_mask[:] = user.color
                ...
                colored_silhouette = cv2.bitwise_and(color_mask, color_mask, mask=mask)
